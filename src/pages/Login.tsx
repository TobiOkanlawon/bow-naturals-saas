...
import Input from "@/components/shared/PasswordInput";
...
const plans = [
  ...
  },
  {
    ...
    features: [
      ...
      "Custom domain",
      "Omnichannel sync (Elementor, GHL, Shopify)",
...
export default function Login() {
  ...
  const handleSignup = async () => {
    ...
    const result = await register(
      ...
      signupName,
      signupBusiness,
      `${capitalize(selectedPlan)} -- ${capitalize(billing)}`,
    );
  ...
  if (signupSuccess) {
    return (
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #312E81, #4F46E5, #818CF8)",
        }}
      >
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            Welcome to {signupBusiness}!
          </h2>
          <p className="text-gray-500 text-sm mb-2">
            Your workspace is ready with a{" "}
            <strong>{selectedPlanData.trial}</strong> trial.
          </p>
          <p className="text-xs text-gray-400 mb-6">
            Plan: {selectedPlanData.name} • ₦
            {getPrice(selectedPlanData, billing).toLocaleString()}
            {getPeriod(billing)}
          </p>
          <button
            onClick={() => {
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
            ...
            <p
              ...
              style={{ color: selectedPlanData.color }}
            >
              ₦{getPrice(selectedPlanData, billing).toLocaleString()}
            ...
            <p className="text-[10px] text-green-600 mb-4">
              Includes {selectedPlanData.trial} — no charge until trial ends
            </p>
            <div className="bg-gray-50 rounded-lg p-3 mb-4 text-left text-[10px] font-mono text-gray-500">
              PaystackPop.setup({"{"} key: 'pk_live_xxx', email: '{signupEmail}
              ', amount: {getPrice(selectedPlanData, billing) * 100}, currency:
              'NGN' {"}"})
            </div>
            <button
              onClick={handlePaystackDemo}
              disabled={loading}
              className="btn-primary w-full py-2.5 bg-green-600 mb-2"
            >
              {loading ? (
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto" />
              ) : (
                `✓ Start ${selectedPlanData.trial}`
              )}
            </button>
            <button
          ...
          {view === "plans" && (
            <div>
              ...
              <p className="text-xs text-gray-500 text-center mb-4">
                All plans include a free trial. No credit card required to
                start.
              </p>
                ...
                {plans.map((plan) => {
                  ...
                  return (
                    ...
                    >
                      {plan.popular && (
                        <div
                          className="absolute -top-2.5 left-1/2 -translate-x-1/2 badge text-white text-[9px] px-2.5"
                      ...
                      <div className="mb-3">
                        ...
                        {savings > 0 && (
                          <span className="ml-1 badge text-[9px] bg-green-50 text-green-700">
                            -{savings}%
                          </span>
                        )}
                      </div>
          ...
          {view === "signup" && (
            <div>
              ...
              <div
                ...
                style={{
                  backgroundColor: `${selectedPlanData.color}08`,
                  border: `1px solid ${selectedPlanData.color}20`,
                }}
              >
                <div
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-gray-500">
                    ₦{getPrice(selectedPlanData, billing).toLocaleString()}
                    {getPeriod(billing)} • {selectedPlanData.trial}
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="btn-primary w-full mt-4 py-2.5 flex items-center justify-center"
                style={{ backgroundColor: selectedPlanData.color }}
              >
                {loading ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
